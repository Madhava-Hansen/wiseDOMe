...
function searchSubmit(e) {
  ...
  let fullAddress = window.$w('.search').value();
  let fullAddressSplit = fullAddress.split(" ");
  fullAddressSplit.pop();
  let splitAddress = fullAddressSplit.join("").split("");
  let length = splitAddress.length;
  if (splitAddress[length - 1] === ",") {
    fullAddress =  fullAddressSplit.join(" ").slice(0, fullAddressSplit.join(" ").length - 1)
  } else {
    fullAddress = fullAddressSplit.join(" ");
  }
  window.$w('.city-name').html(fullAddress);
  setDayAndTime();
